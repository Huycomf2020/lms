(() => {
  ...
  async function saveBulk() {
    const valid=staged.filter(x=>!x.errors.length); if(!valid.length)return;
    const button=$("#save-bulk"); button.disabled=true; msg(`Đang lưu ${valid.length} câu hỏi…`);
    try {
      const items=valid.map(x=>({subject_id:$("#bulk-subject").value,code:x.code,stem:x.stem,options:x.options,cognitive_level:x.cognitive_level,topic:x.topic,status:"draft"}));
      ...
      msg(`Đã lưu ${saved.length} câu ở trạng thái bản nháp. Chưa câu nào được tự động duyệt.`); staged=[]; $("#import-preview").hidden=true; render("questions");
    } catch(error) { msg(`Lưu chưa hoàn tất: ${error.message}`); } finally { button.disabled=false; }
  }
  ...
  $("#parse-ai").onclick=()=>{try{const raw=$("#ai-output").value.replace(/^```(?:json)?|```$/gmi,"").trim();const data=JSON.parse(raw);if(!Array.isArray(data))throw new Error("Kết quả phải là một mảng JSON");stage(data)}catch(error){msg(`JSON chưa đúng: ${error.message}`)}};
